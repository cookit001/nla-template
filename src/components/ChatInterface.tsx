...
import { Sparkles, Send, Bot, User, Copy, Check, Share2, Download, ShieldCheck, AlertOctagon, RotateCcw } from 'lucide-react';
...
export function ChatInterface() {
  ...
  return (
    <div className="w-full max-w-full flex flex-col h-[580px] sm:h-[640px] glass-panel rounded-2xl border border-slate-800/80 shadow-2xl overflow-hidden relative">
      {/* Top Header Bar */}
      <div className="bg-slate-950/90 border-b border-slate-800/80 px-4 py-3 flex items-center justify-between z-10 shrink-0">
        <div className="flex items-center space-x-2.5">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-tr from-amber-500 to-amber-300 flex items-center justify-center shadow-lg shadow-amber-500/20 text-slate-950">
        ...
        <button
          ...
          className="text-slate-400 hover:text-amber-400 p-1.5 rounded-lg hover:bg-slate-900 transition text-xs flex items-center space-x-1"
          title="Reset Chat"
        >
      ...
      {/* Messages Feed */}
      <div className="flex-1 overflow-y-auto p-3 sm:p-5 space-y-4 max-w-full">
        {messages.map((msg) => (
          <div
            key={msg.id}
          ...
          >
            {/* AI Avatar */}
            {msg.sender === 'ai' && (
              <div className="w-7 h-7 rounded-lg bg-slate-900 border border-amber-500/30 flex items-center justify-center text-amber-400 shrink-0 mt-1">
                <Bot className="w-4 h-4" />
              </div>
            )}

            {/* Message Bubble Container */}
            <div className={`flex flex-col space-y-2 max-w-[88%] sm:max-w-[80%] ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
              <div
              ...
              >
                ...
                {msg.renderedText && (
                  <div className="mt-3 bg-slate-950/90 border border-amber-500/30 rounded-xl p-3.5 space-y-3 max-w-full overflow-hidden text-left font-mono">
                    <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                      <div className="flex items-center space-x-1.5 text-[10px] text-amber-400 font-bold tracking-wider">
                        <ShieldCheck className="w-3.5 h-3.5" />
                        <span>MUTUAL NDA • {msg.data?.governingJurisdiction || 'Delaware, USA'}</span>
                      </div>
                      <span className="text-[9px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 px-1.5 py-0.5 rounded">
                        HASH VERIFIED
                      </span>
                    </div>

                    <div className="text-[10px] sm:text-xs text-slate-300 leading-relaxed whitespace-pre-wrap max-h-48 overflow-y-auto p-2 bg-slate-900/50 rounded-lg">
                      {msg.renderedText}
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 pt-1">
                      <button
                        ...
                        className="flex items-center space-x-1 bg-slate-800 hover:bg-slate-700 text-slate-200 px-2.5 py-1 rounded-md text-[11px] font-semibold transition"
                      >
                        {copiedId === msg.id ? (
                      ...
                      <button
                        ...
                        className="flex items-center space-x-1 bg-purple-950 hover:bg-purple-900 text-purple-200 border border-purple-500/30 px-2.5 py-1 rounded-md text-[11px] font-semibold transition"
                      >
        ...
        {loading && (
          <div className="flex items-center space-x-2.5 animate-pulse">
            <div className="w-7 h-7 rounded-lg bg-slate-900 border border-amber-500/30 flex items-center justify-center text-amber-400 shrink-0">
              <Bot className="w-4 h-4" />
            </div>
      ...
      <div className="bg-slate-950/95 border-t border-slate-800/80 p-3 space-y-2.5 shrink-0 z-10">
        ...
        <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 no-scrollbar">
          {quickPrompts.map((item, idx) => (
            <button
              ...
              className="bg-slate-900/80 hover:bg-amber-500/10 border border-slate-800 hover:border-amber-500/40 text-slate-300 hover:text-amber-300 px-2.5 py-1 rounded-lg text-[10px] font-medium whitespace-nowrap transition duration-200 shrink-0"
            >
              {item.label}
            </button>
          ))}
        ...
        <div className="flex items-end space-x-2">
          <textarea
            ...
            className="flex-1 glass-input rounded-xl px-3.5 py-2.5 text-xs sm:text-sm text-slate-100 placeholder-slate-500 focus:outline-none resize-none max-h-24 leading-relaxed"
          />

          <button
            onClick={() => handleSendMessage()}
            disabled={!input.trim() || loading}
            className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500 to-amber-400 hover:from-amber-400 hover:to-amber-300 text-slate-950 font-bold flex items-center justify-center transition shadow-md shadow-amber-500/20 disabled:opacity-40 shrink-0"
          >
